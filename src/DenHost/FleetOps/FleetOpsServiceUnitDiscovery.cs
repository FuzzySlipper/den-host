...
public interface IFleetOpsServiceUnitDiscovery
{
    ...
    Task<(IReadOnlyList<FleetServiceUnit> Units, string? Diagnostics)> DiscoverAsync(
        CancellationToken cancellationToken = default);
}

/// <summary>
/// Discovers service units by querying systemctl for hermes-gateway@* services.
/// Tries --user manager first, falls back to --system.
/// </summary>
public sealed class SystemdFleetOpsDiscovery : IFleetOpsServiceUnitDiscovery
{
    ...
    public async Task<(IReadOnlyList<FleetServiceUnit> Units, string? Diagnostics)> DiscoverAsync(
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    private async Task<(List<FleetServiceUnit> Units, string? Diagnostics)> QuerySystemctlAsync(
        string mode, CancellationToken ct)
    {
        try
        {
            ...
            if (process.ExitCode != 0)
                return (new List<FleetServiceUnit>(),
                    $"systemctl {mode} exit code {process.ExitCode}: {Truncate(stderr, 200)}");
...
public sealed class StubFleetOpsDiscovery : IFleetOpsServiceUnitDiscovery
{
    ...
    public StubFleetOpsDiscovery(IReadOnlyList<FleetServiceUnit>? units = null,
        string? diagnostics = null, bool simulateFailure = false)
    {
    ...
    public Task<(IReadOnlyList<FleetServiceUnit> Units, string? Diagnostics)> DiscoverAsync(
        CancellationToken cancellationToken = default)
    {
        if (_simulateFailure)
            return Task.FromResult<(IReadOnlyList<FleetServiceUnit> Units, string? Diagnostics)>(
                (Array.Empty<FleetServiceUnit>(), _diagnostics ?? "Simulated discovery failure"));
...
public sealed class NoopFleetOpsDiscovery : IFleetOpsServiceUnitDiscovery
{
    public Task<(IReadOnlyList<FleetServiceUnit> Units, string? Diagnostics)> DiscoverAsync(
        CancellationToken cancellationToken = default)
        => Task.FromResult<(IReadOnlyList<FleetServiceUnit> Units, string? Diagnostics)>(
            (Array.Empty<FleetServiceUnit>(), (string?)null));
}
