...
public sealed partial class FleetOpsService
{
    ...
    /// </summary>
    public async Task<FleetOpsOverviewResponse> GetOverviewAsync(
    ...
    /// </summary>
    public async Task<FleetOpsActionRun> ExecuteActionAsync(
        string actionId, FleetOpsActionRunRequest request,
        CancellationToken cancellationToken = default)
    {
        var now = DateTimeOffset.UtcNow;
        ...
        {
            return CreateErrorRun(runId, actionId, request, now,
                $"Action '{actionId}' is disabled: {action.DisabledReason}");
        }
        ...
        if (action.NeedsConfirmation)
        {
            ...
            if (!IsConfirmationValid(confirmation, action))
            {
                ...
                return CreateErrorRun(runId, actionId, request, now,
                    "Confirmation is required for this action. Provide a 'confirmation' field.");
            }
        }
        ...
        {
            return CreateErrorRun(runId, actionId, request, now,
                $"Action '{actionId}' has no executable or systemctl template configured");
        }
        ...
        if (RequiresRestartActionGate(action, isDryRun))
        {
            ...
            if (_restartActionCooldownUntil > now)
            {
                var remainingSeconds = Math.Max(1,
                    (int)Math.Ceiling((_restartActionCooldownUntil - now).TotalSeconds));
        ...
        try
        {
            ...
            if (isDryRun && action.Mutating &&
                action.DryRunScriptPath is null && action.DryRunSystemctlTemplate is null)
            {
            ...
            try
            {
                result = await _executor.ExecuteAsync(
                    executable!, argv!, action.TimeoutSeconds, cancellationToken);
            }
            catch (Exception ex)
            {
            ...
            var finalStatus = isDryRun ? "completed" :
                (result.ExitCode == 0 ? "completed" : "failed");
            if (result.ErrorMessage is not null)
                finalStatus = "failed";

            _runStore.UpdateRun(runId, result.ExitCode, finalStatus,
                result.StdoutLines, result.StderrLines, result.ErrorMessage);
    ...
    private static FleetOpsActionRun CreateErrorRun(string runId, string actionId,
        FleetOpsActionRunRequest request, DateTimeOffset now, string error)
    {
    ...
    private static string? ValidateArgs(FleetOpsAction action,
        IReadOnlyDictionary<string, string> args)
    {
        foreach (var schema in action.ArgsSchema)
        {
            var hasValue = args.TryGetValue(schema.Name, out var value) &&
        ...
        // Reject unknown args
        var allowedNames = new HashSet<string>(action.ArgsSchema.Select(a => a.Name),
    ...
    internal (string Executable, string[] Args)? BuildCommand(
        FleetOpsAction action, IReadOnlyDictionary<string, string> args, bool isDryRun)
    {
    ...
    private static string? SubstituteTemplate(string template,
        IReadOnlyDictionary<string, string> args)
    {
        var result = template;
    ...
    private static bool IsConfirmationValid(string confirmation, FleetOpsAction action)
    {
        ...
        return lower is "yes" or "true" or "confirm" or "confirmed" ||
               (action.ConfirmationCopy is not null &&
                string.Equals(confirmation.Trim(), action.ConfirmationCopy,
