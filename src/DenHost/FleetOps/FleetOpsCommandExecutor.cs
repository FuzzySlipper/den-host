...
public interface IFleetOpsCommandExecutor
{
    ...
    /// </summary>
    Task<CommandResult> ExecuteAsync(string executable, string[] args, int timeoutSeconds,
...
public sealed class ProcessFleetOpsCommandExecutor : IFleetOpsCommandExecutor
{
    ...
    public ProcessFleetOpsCommandExecutor(FleetOpsOptions options,
        ILogger<ProcessFleetOpsCommandExecutor> logger)
    {
        _options = options ?? throw new ArgumentNullException(nameof(options));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    }

    public async Task<CommandResult> ExecuteAsync(string executable, string[] args,
        int timeoutSeconds, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(ex, "Command execution failed: {Executable}", executable);
...
public sealed class StubFleetOpsCommandExecutor : IFleetOpsCommandExecutor
{
    ...
    public StubFleetOpsCommandExecutor(
        Func<string, string[], int, CancellationToken, Task<CommandResult>>? handler = null)
    {
        _handler = handler ?? ((_, _, _, _) =>
            Task.FromResult(new CommandResult(0, Array.Empty<string>(), Array.Empty<string>())));
    }

    public Task<CommandResult> ExecuteAsync(string executable, string[] args,
        int timeoutSeconds, CancellationToken cancellationToken = default)
